...
def filter_factors(func, ubound):
    ...
    return list(
        map(
            lambda k: foldl(k, mul),
            filter(lambda l: func(l), map(lambda c: factorise(c, plist), range(ubound))),
        )
    )
...
def plot_by_num_factors(df, unique=False, logy=False):
    ...
    for n_fact in df[column].unique():
        ...
        n_df.plot(x="n", y="count", ax=ax, logy=logy, label="{} factors".format(n_fact))
    plt.show()
